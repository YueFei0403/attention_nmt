...
class MyGRUCell(nn.Module):
    ...
    def forward(self, x, h_prev):
        ...
        # FILL THIS IN
        # ------------
        z = torch.sigmoid(self.weight_z(torch.cat((x, h_prev), 1)))  # update gate
        r = torch.sigmoid(self.weight_r(torch.cat((x, h_prev), 1)))  # reset gate
        g = torch.tanh(self.weight_g(torch.cat((x, r * h_prev), 1)))  # candidate state
...
class GRUEncoder(nn.Module):
    def __init__(self, vocab_size, hidden_size, opts):
        ...
        self.embedding = nn.Embedding(vocab_size, hidden_size) # vocab_size x hidden_size
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, annotations):
        ...
        concat = torch.cat((expanded_hidden, annotations), dim=2) # we need annotations for each decoder hidden state
...
class AttentionDecoder(nn.Module):
    ...
    def forward(self, x, h_prev, annotations):
        ...
        context = torch.sum(attention_weights * annotations, dim=1) # sum along all encoder hidden_states
        embed_and_context = torch.cat((embed, context), dim=1) # hidden_size*2 x seq_len x 1 -- fully connected attention network layer
